              ...
              <Card style={{ flex: 1 }}>
                <CardContent style={{ flex: 1 }}>
                  ...
                  <ListSection style={{ flex: 1 }} title="Ingredients">
                    <FlatList
                      style={{ flex: 1 }}
                      data={this.state.ingredients.filter(
                        item => item.id !== this.state.ingredients.length - 1
                      )}
                      keyExtractor={item => item.id.toString()}
                      renderItem={({ item }) => {
                        return <ListItem title={item.text} />;
                      }}
                    />
                    <TextInput
                      style={{ flex: 1 }}
                  ...
                  <Button
                    onPress={() => {
                      this.setState({
                        ingredients: [
                          ...this.state.ingredients,
                          { text: '', id: this.state.ingredients.length },
                        ],
                      });
                    }}>
                    Add ingredient
                  ...
                  <ListSection style={{ flex: 1 }} title="Instructions">
                    <FlatList
                      style={{ flex: 1 }}
                      data={this.state.instructions.filter(
                        item => item.id !== this.state.instructions.length - 1
                      )}
                      keyExtractor={item => item.id.toString()}
                      renderItem={({ item }) => {
                        return <ListItem title={item.text} />;
                  ...
                  <Button
                    onPress={() => {
                      this.setState({
                        instructions: [
                          ...this.state.instructions,
                          { text: '', id: this.state.instructions.length },
                        ],
                      });
                    }}>
                    Add instruction
